   ...
   switch(type)
     {
	   ...
	   case 2: 
           ...
           case 4:{
                   XMLsendProtectSwitchSwitch(RTUSENDFILE,bData->SwitchPos,SNo,CM,type);break;
                 }
...
int XMLsendProtectSwitchSwitch(char * filename,int SwitchPos,int SNo,int CM,int type)
{
    ...
    printf("-------------------------------------------------->PNo:%d == ModNo:%d sPNo:%d rPNo:%d \n");
    sumStr  =(char *)malloc(sizeof(char)*1024*8);
    *sumStr ='\0';
    if(SwitchPos==96)    Status = 1;   //PARALLEL
    if(SwitchPos==16)    Status = 0;   //ACROSS                                   
    strcat(sumStr,"<?xml version='1.0' encoding='UTF-8'?>\n");
    strcat(sumStr,"<AsynPush>\n");
    ...
    sprintf(str,"%d",rPNo);
    strcat(sumStr,"	<RPNo>"); strcat(sumStr,str);strcat(sumStr,"</RPNo>\n");
    sprintf(str,"%d",sPNo);
    strcat(sumStr,"	<SPNo>"); strcat(sumStr,str);strcat(sumStr,"</SPNo>\n");
    sprintf(str,"%d",Status);
    strcat(sumStr,"	<Status>"); strcat(sumStr,str);strcat(sumStr,"</Status>\n");
    strcat(sumStr,"</SwitchReport>\n");
    strcat(sumStr,"</AsynPush>\n");    
    if ((fd = fopen(filename,"w")) == NULL)  
    {  
