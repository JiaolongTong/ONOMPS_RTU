...
/**************************执行操作********************************/   
	sqlite3 *mydb=NULL;
	sql *mysql=NULL;
	int rc=0,rednum=0,flagA=0,flagB=0,UseFlag=0,PortFlagA=0,PortFlagB=0,flagSW=0,existFlagA=0,existFlagB=0,flagStatus=0;
			...
			SQL_lookupPar(mysql,&result,&rednum);
			  ...
			  resp->Group[ErrorSNo].Error_inform = "Error: Don't have such Port\n";
				...
				if(DoSwitchCounter>0 && flagSW==0){
					...
					mysql->mainKeyValue= strSNoA;
					existFlagA=SQL_existIN_db(mysql);  
					if(existFlagA){                                           //SNoA存在
					...
					mysql->mainKeyValue= strSNoB;
					existFlagB=SQL_existIN_db(mysql); 
					if(existFlagB){                                           //SNoB存在
						mysql->filedsName ="Status";
						SQL_lookupPar(mysql,&result,&rednum);
						flagStatus=atoi(result[0]); 
						SQL_freeResult(&result,&rednum);
