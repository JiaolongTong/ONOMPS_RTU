...
void getProtectSwitchParameter(mxml_node_t *root,mxml_node_t *tree,opticalprotect *protectmpar){
	    mxml_node_t *GX,*PN,*PNo,*SNoA,*SNoB;
...
/**************************解析XML消息***************************************/
	    ...
	    intCM = strtoul (CM->child->value.text.string, NULL, 0); 	
            ...
            for (i=0;i<PN;i++){
                    printf("----------requstPx:%d------------\n",protectmpar->Group[i].PNo);              
                    printf("    SNoA:%d------SNoB:%d\n",protectmpar->Group[i].SNoA,protectmpar->Group[i].SNoB);      
            }  
/**************************数据库与指令一致性检查***************************/
	 sqlite3 *mydb;
	 int rc,flag=-1;
         int ErrorSNo=0,intStatus;
	 sql *mysql;
         char *value; 
         char **result;
	 ...
	 mysql->tableName    =  "ProtectGroupTable"; 
         for (i=0;i<PN;i++){
            ...
            flag=ifProtectMatch(protectmpar->Group[i].SNoA,protectmpar->Group[i].SNoB,mysql); 
		...
		intStatus =atoi(result[0]);
		 ...
		 if(!semaphore_v())                                    //V
	            exit(EXIT_FAILURE); 
                  ...
                  if(intStatus!=1)
                     resp->Group[ErrorSNo].Error_inform = "Error: Sqlite don't match -->ready Switch or ready Cancel [Status=2 or Status=0].\n";
                  if(flag!=1)
                     resp->Group[ErrorSNo].Error_inform = "Error: Sqlite don't match -->commend and database not same.\n";
                  ErrorSNo++;
            }
